...
public class CashCardController {
    ...
    private ResponseEntity<Void> createCashCard(@RequestBody CashCard newCashCardRequest, UriComponentsBuilder uriComponentsBuilder) {
        final CashCard saveCashCard = cashCardRepository.save(newCashCardRequest);
    ...
    private ResponseEntity<List<CashCard>> findAll(Pageable pageable) {
        final Page<CashCard> page = cashCardRepository.findAll(PageRequest.of(pageable.getPageNumber(), pageable.getPageSize(), pageable.getSortOr(Sort.by(Sort.Direction.ASC, "amount"))));
